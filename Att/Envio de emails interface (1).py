...
class EmailSenderThread(QThread):
    ...
    def atualizar_mensagem(self, email_destino, mensagem_html):
        # Obtém o nome e o produto correspondentes ao e-mail de destino
        nome, produto = self.obter_nome_produto(email_destino)
        
        # Atualiza a mensagem de e-mail com o nome e o produto
        nova_mensagem = mensagem_html.replace('{nome}', nome).replace('{produto}', produto)
    ...
    def run(self):
        ...
        corpo_email = f"""
            ...
            <title>Envio de E-mails Automatizados</title>
            <style>
                ...
                * {{
                ...
                width: 120px;
                height: 50px;
                background-color: #146D9A; /* Updated button color */
                border-radius: 10px;
                text-align: center;
                color: #f4f4f4;
                margin: 20px auto 0; /* Centralizando o botão e adicionando margem superior */
                display: block; /* Garantindo que o botão ocupe toda a largura disponível */
                box-shadow: 3px 3px 3px rgba(0, 0, 0, 0.25); /* Added box-shadow for button */
                }}
                ...
                img {{
                filter: drop-shadow(2px 2px 4px rgba(0, 0, 0, 0.335));
                }}

                .corpo_principal {{
                background-color: #b8def1d8;
                ...
                padding: 10px;
                border: 3px solid transparent; /* Added transparent border */
                border-radius: 10px;
                box-shadow: 3px 3px 3px #146D9A; /* Updated box-shadow color */
                overflow: auto; /* Adicionando overflow para que o conteúdo adicional seja rolável */
                }}
            </style>
            </head>
            ...
            <div class="corpo_principal">
                <center>
                <div>
                    <img src="https://webcertificados.com.br/wp-content/uploads/2021/05/logo-site-1024x293.png" alt="Logo" style="width: 50%; height: 50%; margin-right: 10px;">
                    <h1>{self.titulo_html}</h1>
                    <hr /> 
                    <p>{self.mensagem_html}</p>
                    <a href="https://api.whatsapp.com/send?phone=553232175259&text=Ol%C3%A1!%20Vim%20pelo%20site%20e%20gostaria%20de%20tirar%20algumas%20d%C3%BAvidas"><button>Saiba mais!</button></a>
                    <div class="social-icons" style="margin-top: 15px;">
                    <a style="width: 20px; height: 20px; object-fit: contain; padding: 2px 0px px 0; margin: 6px; align-self: center; text-decoration: none;" 
                    href="https://facebook.com/certificadosweb"> <img class="social" style="padding-left: 0;" 
                        src="http://webcertificados.com.br/assinaturas/icon%20Face.png" alt="Facebook" width="24" height="24" /> </a> 
                        <a style="width: 20px; height: 20px; object-fit: contain; padding: 2px 0px 0; align-self: center; text-decoration: none;" href="https://instagram.com/webcertificados"> 
                        <img class="social" src="http://webcertificados.com.br/assinaturas/icon%20Instagram.png" alt="Instagram" width="24" height="24" /></a> 
                        <a style="width: 20px; height: 20px; object-fit: contain; padding: 2px 8px 0; align-self: center; text-decoration: none;" 
                        href="http://webcertificados.com.br/assinaturas/icon%20Wpp.png"></a>           
                    </div>
                </div>
                </center>
            </div>
        ...
        for i, email_destino in enumerate(self.destinatarios):
            ...
            nova_mensagem = self.atualizar_mensagem(email_destino, self.mensagem_html)

            # Configuração do e-mail
            msg = MIMEMultipart()
            
            # Mescla o corpo padrão do e-mail com a nova mensagem atualizada
            corpo_com_mensagem = corpo_email.replace(self.mensagem_html, nova_mensagem)
            corpo_mensagem = MIMEText(corpo_com_mensagem, 'html')
...
class EmailSenderApp(QWidget):
    ...
    def init_ui(self, styleMain):
        ...
        layout_intervalo_env.addWidget(self.intervalo_envio_label)
        layout_intervalo_env.addWidget(self.intervalo_envio_edit)
